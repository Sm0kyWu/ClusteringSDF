...
class ObjectImplicitNetworkGrid(nn.Module):
    def __init__(
        ...
        self.sdf_bounding_sphere = sdf_bounding_sphere
        self.sphere_scale = sphere_scale
        # print(if_edge, if_instance, if_confidence)
        # exit(0)
        if if_edge:
            self.d_out_edge = 1  # New output dimension for edge detection
            dims = [d_in] + dims + [d_out + feature_vector_size + self.d_out_edge]
        elif if_instance:
            self.d_out_inst = 2  # New output dimension for instance loss
            dims = [d_in] + dims + [d_out + feature_vector_size + self.d_out_inst]
        elif if_confidence:
            self.d_out_conf = 1
            dims = [d_in] + dims + [d_out + feature_vector_size + self.d_out_conf]
        else:
            dims = [d_in] + dims + [d_out + feature_vector_size]
        self.embed_fn = None
    ...
    def forward(self, input):
        ...
        for l in range(0, self.num_layers - 1):
            ...
            x = lin(x)
            if l != self.num_layers - 2 and self.if_dropout:
                x = self.dropout(x)
            if l < self.num_layers - 2:
                x = self.softplus(x)
    ...
    def get_outputs(self, x, beta=None):
        ...
        if self.if_edge:
            edge_output = torch.sigmoid(output[:, -self.d_out_edge:])
        if self.if_instance:
            first_layer = output[:, -2]
            second_layer = output[:, -1]
            first_layer_sigmoid = torch.sigmoid(first_layer)
            second_layer_sigmoid = torch.sigmoid(second_layer)
            instance_output = torch.stack([first_layer, second_layer], dim=1)
        if self.if_confidence:
            confidence_output = torch.sigmoid(output[:, -self.d_out_conf:])
        ...
        if self.if_edge:
            return sdf, feature_vectors, gradients, semantic, sdf_raw, edge_output
        if self.if_instance:
            return sdf, feature_vectors, gradients, semantic, sdf_raw, instance_output
        if self.if_confidence:
            return sdf, feature_vectors, gradients, semantic, sdf_raw, confidence_output
        return sdf, feature_vectors, gradients, semantic, sdf_raw
...
class RenderingNetwork(nn.Module):
    def __init__(
        ...
        self.if_edge = if_edge
        if self.if_edge:
            dims = [d_in + feature_vector_size + 1] + dims + [d_out]
        elif if_instance:
            dims = [d_in + feature_vector_size + 2] + dims + [d_out]
        elif if_confidence:
            dims = [d_in + feature_vector_size + 1] + dims + [d_out]
        else:
...
class ObjectSDFPlusNetwork(nn.Module):
    def __init__(self, conf):
        ...
        self.num_boundary = 2
        self.if_edge = conf.get_bool('if_edge', default=False)
        self.if_instance = conf.get_bool('if_instance', default=False)
        self.if_confidence = conf.get_bool('if_confidence', default=False)
    ...
    def forward(self, input, indices):
        ...
        # print(points_flat.shape, dirs_flat.shape, z_vals.shape, depth_scale.shape)
        points_flat_extra = None
        dirs_flat_extra = None
        z_vals_extra = None
        depth_scale_extra = None
        if "extra_pose" in input.keys():
            N = input["extra_uv"].squeeze().size(0)
            extra_poses = [input["extra_uv"].squeeze()[i].unsqueeze(0) for i in range(N)]
            for i in range(N):
                extra_ray_dirs, extra_cam_loc = rend_util.get_camera_params(input["extra_uv"].squeeze()[i].unsqueeze(0), input["extra_pose"].squeeze()[i].unsqueeze(0), input["extra_intrinsics"].squeeze()[i].unsqueeze(0))
                extra_dirs_tmp, _ = rend_util.get_camera_params(input["extra_uv"].squeeze()[i].unsqueeze(0), torch.eye(4).to(pose.device)[None], input["extra_intrinsics"].squeeze()[i].unsqueeze(0))
                extra_depth_scale = extra_dirs_tmp[0, :, 2:]
                _, extra_num_pixels, _ = extra_ray_dirs.shape
                extra_cam_loc = extra_cam_loc.unsqueeze(1).repeat(1, extra_num_pixels, 1).reshape(-1, 3)
                extra_ray_dirs = extra_ray_dirs.reshape(-1, 3)
                extra_z_vals, _ = self.ray_sampler.get_z_vals(extra_ray_dirs, extra_cam_loc, self)
                extra_N_samples = extra_z_vals.shape[1]
                extra_points = extra_cam_loc.unsqueeze(1) + extra_z_vals.unsqueeze(2) * extra_ray_dirs.unsqueeze(1)
                extra_points_flat = extra_points.reshape(-1, 3)
                extra_dirs = extra_ray_dirs.unsqueeze(1).repeat(1,extra_N_samples,1)
                extra_dirs_flat = extra_dirs.reshape(-1, 3)

                if points_flat_extra is None:
                    points_flat_extra = extra_points_flat
                    dirs_flat_extra = extra_dirs_flat
                    z_vals_extra = extra_z_vals
                    depth_scale_extra = extra_depth_scale
                else:
                    points_flat_extra = torch.cat([points_flat_extra, extra_points_flat], 0)
                    dirs_flat_extra = torch.cat([dirs_flat_extra, extra_dirs_flat], 0)
                    z_vals_extra = torch.cat([z_vals_extra, extra_z_vals], 0)
                    depth_scale_extra = torch.cat([depth_scale_extra, extra_depth_scale], 0)


        beta_cur = self.density.get_beta()
        if self.if_edge:
            sdf, feature_vectors, gradients, semantic, sdf_raw, edge_output = self.implicit_network.get_outputs(points_flat, beta=None)
        elif self.if_instance:
            sdf, feature_vectors, gradients, semantic, sdf_raw, instance_output = self.implicit_network.get_outputs(points_flat, beta=None)
        elif self.if_confidence:
            sdf, feature_vectors, gradients, semantic, sdf_raw, confidence_output = self.implicit_network.get_outputs(points_flat, beta=None)
        else:
        ...
        if points_flat_extra is not None:
            sdf_extra, _, _, _, sdf_raw_extra = self.implicit_network.get_outputs(points_flat_extra, beta=None)
            weights_extra, transmittance_extra, dists_extra = self.volume_rendering(z_vals_extra, sdf_extra)
            object_opacity_extra = self.occlusion_opacity(z_vals_extra, transmittance_extra, dists_extra, sdf_raw_extra).sum(-1).transpose(0, 1)
        ...
        if self.if_edge:
            edge_values = torch.sum(weights.unsqueeze(-1) * edge_output.reshape(-1, N_samples, 1), 1)

        if self.if_instance:
            instance_values = torch.sum(weights.unsqueeze(-1) * instance_output.reshape(-1, N_samples, 2), 1)

        if self.if_confidence:
            confidence_values = torch.sum(weights.unsqueeze(-1) * confidence_output.reshape(-1, N_samples, 1), 1)
        ...
        output = {
            ...
            'weights': weights,
            'edge_values': edge_values if self.if_edge else None,
            'instance_values': instance_values if self.if_instance else None,
            'confidence_values': confidence_values if self.if_confidence else None,
        }

        if points_flat_extra is not None:
            output['object_opacity_extra'] = object_opacity_extra
